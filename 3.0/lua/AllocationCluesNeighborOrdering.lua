function AllocationCluesNeighborOrdering (component)
			...
			for k,cell in pairs (cs.cells) do
				...
				if k > 1 then
					for i, lu in  pairs (luTypes) do
							...
							potentialCell = (1 + Cell[lu.."_pot"]["tau_"..lu]) * Cell[lu.."_pot"]["suit_"..lu]
							potentialupCell = (1 + upCell[lu.."_pot"]["tau_"..lu]) * upCell[lu.."_pot"]["suit_"..lu]
							
							while (potentialCell > potentialupCell) do 
								aux = Cell[lu.."_pot"]
								Cell[lu.."_pot"] = upCell[lu.."_pot"]
								upCell[lu.."_pot"] = aux
		...
		while ((nIter <= max_iteration) and (allocation_ok == false) ) do	
			...
			for k,cell in pairs( cs.cells ) do
				...
				for i, lu in  pairs( luTypes ) do	
						...
						suit_plus_iter = (1 + cell[lu.."_pot"]["tau_"..lu]) * cell[lu.."_pot"]["suit_"..lu] +  iteration[lu]
  ...
  component.calcDifferences = function(self,event,luccMEModel)
    ...
    local dem = 0
  	...
  	for luind, land in pairs (luTypes) do
  		...
  		dem = demand:getDemand(luccMEModel,time,land)
  ...
  component.adjustIteration = function(cs, diff, luTypes, faciter, iter, cellarea, maxdiffarea )	 		
  	for luind, land in pairs( luTypes) do
