...
System::Void LuccME::NovoModelo::NovoModelo_Load(System::Object ^ sender, System::EventArgs ^ e)
{
	...
	if (lOpen) {
		...
		if (mainFile->ShowDialog() == System::Windows::Forms::DialogResult::OK)
		{
			...
			line = sw->ReadLine();
			while (line->Contains("cellArea") != 1) {
				line = sw->ReadLine();
			}
			...
			line = sw->ReadLine();
			while (line->Contains("landUseNoData") != 1) {
				line = sw->ReadLine();
			}
			...
			line = sw->ReadLine();
			while (line->Contains("outputTheme") != 1) {
				line = sw->ReadLine();
			}
			...
			line = sw->ReadLine();
			while (line->Contains("saveAttrs") != 1) {
				line = sw->ReadLine();
			}
