...
CellularSpace_ = {
  ...
  -- "coord" & A bidirected relation between two CellularSpaces connecting Cells with the same (x, y) coordinates. & target & name\
  -- "function" & A Neighborhood based on a function where any other Cell can be a neighbor. & filter & name, weight \
  -- "moore"(default) & A Moore (queen) Neighborhood, connecting each Cell to its (at most) eight touching Cells. & & name, self, wrap \
  -- "mxn" & A m (columns) by n (rows) Neighborhood within the CellularSpace or between two CellularSpaces if target is used. & m & name, n, filter, weight, target \
  -- "vonneumann" & A von Neumann (rook) Neighborhood, connecting each Cell to its (at most) four ortogonally surrounding Cells. & & name, self, wrap
  ...
  -- @param data.name A string with the name of the Neighborhood to be created. The default is "1".
  ...
  -- @param data.wrap Whether Cells in the borders will be connected to the Cells in the opposite border. Default is false.
  ...
  -- }
	createNeighborhood = function(self, data)
		...
		--@RAIAN: Verificando se a vizinhanca ja existe para emitir Warning
		if(self.cells[1] and #self.cells[1] >= 1)then
			forEachNeighborhood(
				self.cells[1],
				function(neigh)
					if(neigh:getId() == data.name)then
						customWarningMsg("Warning: The Neighborhood '"..data.name.."' already exists in this CellularSpace. It will be overwritten.", 5)
					end
				end
			)
		end
	...
	-- @param xIndex A X coord.
	-- @param yIndex A Y coord.
	-- @usage cs:getCell(x,y)
	getCell = function(self, xIndex, yIndex)
		if type(xIndex) ~= "number" or math.floor(xIndex) ~= xIndex then
			if xIndex == nil then
		...
		if type(yIndex) ~= "number" or math.floor(yIndex) ~= yIndex then
			if yIndex == nil then
				deniedOperationMsg("getCell", 3)
				return nil
			else
  ...
  --- Load the CellularSpace from the database. TerraME automatically executes this function when the CellularSpace is created, but one can execute this to load the attributes again, erasing each other attribute and relations created by the modeler. This function can not be used with volatile CellularSpace.
  -- @usage cs:load()
	load = function(self)
        ...
        data[i].id = "cell".. globalCellIdCounter
...
-- Every Cell of a CellularSpace has an (x, y) location, starting from (0, 0). This Cell represents the
-- upper right location.  Note that CellularSpaces loaded from
-- databases might not have this cell, according to the original geometry that was used to create the CellularSpace.
...
-- @param data.autoload A boolean value indicating whether the CellularSpace will be loaded automatically (true, default value) or the user by herself will call load (false).
...
-- @param data.where An SQL restriction on the properties of the Cells (default is "", applying no restriction. Only the Cells that reflect the established criteria will be loaded). For example, the operator to compare value is "=" and not "==". This parameter can only be used when reading data from a database.
...
--
	...
	-- Access & Load from a Microsoft Access database (.mdb)  file. & database, theme & layer, select, where \
	--CSV & Load  from a Comma-separated value (.csv) file. Each column will become an attribute. It requires at least two attributes: x and y. & database & sep\
	-- MySQL & Load from a TerraLib database stored in a MySQL database. & database, theme & host, layer, password, port, select, user, where \
	--Shapefile & Load data from a shapefile. It requires three files with the same name and different extensions: .shp, .shx, and .dbf. The argument database must contain the file with extension .shp.& database & \
  -- Volatile & Create a rectangular cellular space from scratch. Cells will be instantiated with only two attributes: x and y. & dxim & ydim 
...
function CellularSpace(data)
	if(type(data) ~= "table")then
 	...
 	elseif(getn(data) == 0)then
 		customErrorMsg("Error: 'CellularSpace' needs more informations to be created.", 3)
	end
