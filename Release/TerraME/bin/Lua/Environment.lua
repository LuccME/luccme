...
Environment_ = { 
	...
	add = function (self, object)
		local t = type(object)
		if t == "CellularSpace" or t == "Society" or t == "Agent" or t == "Automaton" or t == "Timer" or t == "Trajectory" or t == "Cell" then
			object.parent = self
			table.insert(self, object)
		else
      		incompatibleTypesErrorMsg("#1","Agent, Automaton, Cell, CellularSpace, Society, Timer or Trajectory", t, 3)
    	end
		object.parent = self
	...
	--- Create relations between behavioural entities (Agents) and spatial entities (Cells). The Environment must have only one CellularSpace or Trajectory to place agents. It is possible to have more than one behavioural entity in the Environment. When distributing Agents over a Trajectory, the Agents will be able to move over the whole CellularSpace.
	...
	-- @param data.name A string representing the name of the relation in TerraME objects. Default is "placement", which means that the modeler can use Agent:enter(), Agent:move(), and Agent:leave() directly. If the name is different from the default value, the modeler will have to use the last argument of these functions to indicate which relation they are changing or perform changes on these relations manually.
	...
	createPlacement = function(self, data)
		if(type(data) ~= "table")then
			if(data == nil)then
				data = {}
	 			...
	 			namedParametersErrorMsg("createPlacement", 3)
			end
		end
		...
		for k, ud in pairs(self) do
			...
			if t == "CellularSpace" or t == "Trajectory" then
				if mycs ~= nil then
					customErrorMsg("Error: Environment should have only one CellularSpace or Trajectory.", 3)
				end
				mycs = ud
	...
	-- @param data.name A string representing the name of the relation to be created. Default is '1'.
	-- @param data.bidirect If 'true' then for each relation from Cell a to Cell b, create also a relation from b to a. Default is 'false'.
...
function Environment(data)
	if(type(data) ~= "table")then
		if(data == nil)then
			tableParameterErrorMsg("Environment", 3)
		else
			namedParametersErrorMsg("Environment", 3)
		end
	end
	...
	for k, ud in pairs(data) do
		...
		if(t=="Timer") then
			-- RAIAN: Adicionei isto pq nao estava setando o parent para objetos do tipo Timer
			ud.parent = data
			cObj:add(ud.cObj_)
		end
	end
