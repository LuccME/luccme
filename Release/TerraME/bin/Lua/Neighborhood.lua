...
Neighborhood_ = {
	...
	--#- Add a new Cell to the Neighborhood.
	-- It returns a boolean value indicating whether the Cell was correctly added.
	-- @param xIndex number A X coord where a cell will be added.
	-- @param yIndex number A Y coord where a cell will be added.
	...
	-- @usage n:addCell(x, y, cs)
	-- n:addCell(x, y, cs, 0.001)
	addCell = function(self, xIndex, yIndex, cellularSpace, weight)
		...
		elseif(type(cellularSpace) ~= "CellularSpace")then
			incompatibleTypesErrorMsg("#3", "CellularSpace", type(cellularSpace), 3)
		end

		if(weight == nil)then
			mandatoryArgumentErrorMsg("#4", 3)
		elseif(type(weight) ~= "number")then
			incompatibleTypesErrorMsg("#4", "number", type(weight), 3)
		end
	...
	-- It returns a boolean value indicating whether the Cell was correctly added.
	-- @param xIndex A X coord where a cell will be removed.
	-- @param yIndex A Y coord where a cell will be removed.
	-- @usage n:eraseCell(x,y)
	...
	-- @param xIndex A X coord where a cell is located.
	-- @param yIndex A Y coord where a cell is located.
	...
	-- @param xIndex A X coord where a Cell will be updated.
	-- @param yIndex A Y coord where a Cell will be updated.
	-- @param cell A Cell.
	-- @usage n:setCell(x, y, cell)
	setCell = function(self,xIndex, yIndex, cell)
		if(xIndex == nil)then
			mandatoryArgumentErrorMsg("#1", 3)
		elseif(type(xIndex) ~= "number" or math.floor(xIndex) ~= xIndex)then
			incompatibleTypesErrorMsg("#1", "positive integer number", type(xIndex), 3)
    ...
    local index = Coord{ x=xIndex, y=yIndex}
		...
		if(cell == nil)then
			mandatoryArgumentErrorMsg("#3", 3)
		elseif(type(cell) ~= "Cell")then
			incompatibleTypesErrorMsg("#3", "Cell", type(cell), 3)
		end
	...
	getCellNeighbor = function(self, xIndex, yIndex)
		if(xIndex == nil)then
			mandatoryArgumentErrorMsg("#1", 3)
		elseif(type(xIndex) ~= "number" or math.floor(xIndex) ~= xIndex) then
			incompatibleTypesErrorMsg("#1", "positive integer number", type(xIndex), 3)
	...
	--- Retrieve a positive integer number representing the 'x' coord the neighbor pointed by the current iterator.
	--@usage x = n:getX()
	getX = function(self)
    return self.cObj_:getX()
	end,

	--- Retrieve a positive integer number representing the 'y' coord the neighbor pointed by the current iterator.
	--@usage x = n:getY()
	getY = function(self)
		return self.cObj_:getY()
	end,
	...
	--@param xIndex A Coord where a Cell is located.
	--@param yIndex A Coord where a Cell is located.
	--@param weight A number representing the new weight.
	--@usage n:setCellWeight(x, y, 0.001)
	...
	--@usage print(#n)
	size = function(self)
		customWarningMsg("Warning: Function 'size()' is deprecated. Use operator # instead.", 3)
		return #self
	end,
	-- RAIAN
...
--- Each Cell has one or more Neighborhoods to represent proximity relations. A Neighborhood is a set of pairs (cell, weight), where cell is a neighbor Cell and weight is a number storing the relation's strength. This type is used to create neighborhoods from scratch to be used by Cell:addNeighborhood(). To create well established neighborhoods see CellularSpace:createNeighborhood(). Neighborhoods can also be loaded from external soures using CellularSpace:loadNeighborhood().
--Calling Utils:forEachNeighbor()
	...
	if(type(data) ~= "table")then
		if(data == nil)then 
			data = {} 
 			...
 			namedParametersErrorMsg("Neighborhood", 3)
		end
	end

	--@RAIAN: Setando o id default da vizinhanca (Aqui nao havia sido alterado)
	if(data.id == nil) then
