Timer_ = {
	...
	execute = function(self, finalTime)
 	   if finalTime == nil then
			finalTime = 1
			defaultValueWarningMsg("#1", "number", finalTime, 3)
	...
	-- @param modelTime An positive integer number representing time to be used by the Observer. Default is the current simulation time 'self:getTime()'.
	-- @usage timer:notify()
